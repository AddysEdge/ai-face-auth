...
B4, **B4a**, B5-B14, B15, **B16**, **B17**, and **B18**. Writing "B1-B15"
silently omits B4a, B16, B17, and B18, so refer to it as *"every Part B entry
...
| **B17** | **The verification path makes no outbound network connections.** Phase 3 specifies the verifier service as having no network access at all (section 3.4 of `docs/PHASE2_SECURITY_REVIEW.md`, ADR-0002 section 5.3). The current dependency set cannot satisfy that: the bundled MediaPipe binary opens a TLS connection to `play.googleapis.com` and uploads usage telemetry on session teardown, upstream provides no supported way to disable it, and the behaviour is documented and intended (`docs/PRIVACY_NETWORK_AUDIT.md`). **This is a design conflict, not a documentation problem.** ADR-0005 must move from *Proposed* to *Accepted* with **Option A** (replace MediaPipe) or **Option B** (transparently build and verify MediaPipe without telemetry) selected **and implemented**. Those are the only two resolutions. The mandatory interim disclosure already applied in Phase 1 - retracting the false offline claims and documenting the actual behaviour - makes the documentation truthful, but it is not a resolution and does **not** clear this criterion. | `scripts/check_network_activity.py` run against the Phase 3 verification path, showing **zero** outbound connections, with `scripts/network_allowlist.json` empty. Plus ADR-0005 recorded as Accepted with the chosen option and its evidence. Neither a firewall rule, a hosts-file entry, nor any other per-machine block counts: the requirement is a property of the software, not of the machine it happens to run on. **Status after Phase 2.5 (2026-08-31, including the post-merge correction): CLEARED.** Option A was implemented: the liveness path drives the same pinned `face_landmarker.task` weights on `ai-edge-litert` through a reimplementation of MediaPipe's published pipeline, and the `mediapipe` dependency is removed. Against the 1.0.1 oracle across 46 synthetic cases, with every metric **enforced** by the harness's exit status against limits declared before the evidence was regenerated: landmark 0.92339 px (limit 1.0 px), blink 0.01363 (0.02), head-turn ratio 0.00298 (0.0045), each of the 52 blendshapes 0.02779 (0.05), detection agreement 46/46, and face-presence-gate agreement 44/44. An earlier revision called Option A "contradicted by measurement"; that is withdrawn - the replica it rested on used `warpAffine` with a zero border rather than the published `warpPerspective`/`BORDER_REPLICATE` path, and fed the blendshape model normalized rather than image-denormalized landmarks. `scripts/network_allowlist.json` is now empty and 20 fresh-process FULL-mode runs of `scripts/check_network_activity.py` observed **zero** external endpoints, each with the loopback canary proven, 5-6 successful OS queries, zero failed queries and no expired deadline (`docs/b17/network_silence_20_runs.json`). ADR-0005 is Accepted. Thresholds were not changed and no firewall, hosts-file or per-machine block was used. **Scope of this clearance.** B17 is about *network silence* and nothing else. It does **not** assert that the replacement runtime has demonstrated equivalent FAR/FRR or spoof resistance: the synthetic corpus cannot exercise `blink_score_high = 0.40` at all, because MediaPipe itself emits at most ~0.21 on procedurally drawn faces. That question is tracked separately as **B18**, which is OPEN. **Remaining limitation:** the network check is a detector, not a proof of absence - a connection shorter than the poll interval could be missed, and it observes `IP:port`, never payload bytes. **Post-merge correction (2026-08-31):** an audit found the landmark model's face-presence gate was missing from the replacement, and the oracle comparison was non-enforcing. Both are fixed and the network evidence was regenerated against the corrected shipping code; see `docs/PHASE2_5_B17_RESEARCH.md`. The criterion itself is unchanged. |
| **B18** | **Real-input liveness replacement validation.** The challenge-response liveness check is a security control, and Phase 2.5 replaced the runtime that computes its inputs. B17 established that the replacement is network-silent and that it agrees with the runtime it replaced on a synthetic corpus, to declared numeric tolerances. Neither of those is evidence that the control still *works*: synthetic agreement on operator outputs says nothing about false-accept and false-reject rates, and nothing about spoof resistance. The corpus provably cannot reach the configured decision thresholds - MediaPipe itself emits a maximum blink score of about 0.21 on procedurally drawn faces, against a `blink_score_high` of 0.40 - so the thresholds the control actually uses have never been exercised end to end on the replacement. This criterion closes that gap. It is **not** satisfied by any amount of additional synthetic measurement. | Evidence covering, at minimum: **(a)** genuine blinks and genuine non-blinks from real input; **(b)** the configured thresholds `blink_score_high = 0.40` and `blink_score_low = 0.20` actually exercised, with observed score distributions on both sides; **(c)** measured false-accept and false-reject behaviour, with the sample size and confidence stated; **(d)** static printed-photo attacks; **(e)** display and video-replay behaviour, including the known replay limitation recorded in `docs/THREAT_MODEL.md`; **(f)** variation across lighting, head pose, subject distance, and at least one camera other than the development one; **(g)** a written calibration methodology, so the numbers are reproducible rather than anecdotal; and **(h)** an explicit security review of the result, recorded as a decision rather than an observation. Collecting this requires a camera, real people, informed consent, and a data-handling plan for the recordings - none of which existed during Phase 2.5, which is why the criterion is separate rather than folded into B17. Tracked in [issue #14](https://github.com/AddysEdge/ai-face-auth/issues/14). A proposed validation protocol and data-handling plan is in [`docs/B18_REAL_INPUT_VALIDATION_PLAN.md`](B18_REAL_INPUT_VALIDATION_PLAN.md); it is **awaiting owner decision and does not clear this criterion** - no capture may begin until the participant, consent, storage, retention and deletion decisions in its section 4 are recorded, and owner-only pilot evidence is explicitly insufficient for any general security claim. **Status: OPEN.** |
