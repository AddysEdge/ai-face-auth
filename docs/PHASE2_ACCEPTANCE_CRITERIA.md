...
| A9 | Any newly added tests pass | **Met** | native CTest suite, 25 tests, Debug + Release |
...
| A13 | GitHub Actions pass, or limitations documented honestly | **Met** | `.github/workflows/ci.yml`, `.github/workflows/codeql.yml`; toolchain gap documented in Part D |
...
**All of B1-B14 must be true. Any single failure blocks Phase 3.**
...
Phase 3 must never: register the provider as the sole sign-in option; populate
the `Exclude` list; disable or hide the password provider or Windows Hello;
handle a Windows password in any form; modify LogonUI, Winlogon, LSA,
Credential Guard, Windows Hello, provider filters, authentication policies, or
account settings beyond the provider's own additive registration; use
undocumented NGC or Windows Hello internals; report a successful Windows
authentication based only on a face match; or run experiments on a real lock
