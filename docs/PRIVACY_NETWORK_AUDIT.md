...
**Updated by Phase 2.5 (2026-08-27):** Option A was implemented as an
independent reimplementation and is **contradicted by measurement** - the blink
score diverges by up to 0.110 against a 0.20-wide decision band, and the error
persists with MediaPipe's own ROI, so the cause is crop resampling rather than
the ROI arithmetic. Option B is **available and unverified**, and is now the
leading candidate; an earlier revision rejected it on a requirement that B17
does not contain, and that rejection is withdrawn. B17 remains open. Full
